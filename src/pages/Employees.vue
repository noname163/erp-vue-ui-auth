<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <RequireRole :roles="['admin','manager']">
          <button class="btn btn-secondary h-9 px-3" @click="openImport = true">{{ t('employees.importCsv') }}</button>
          <button class="btn btn-primary h-9 px-3" @click="openCreate = true">{{ t('employees.add') }}</button>
        </RequireRole>
      </div>
    </div>
    ...
    <AppModal :open="openCreate" :title="t('employees.createTitle')" :confirmText="t('controls.create')" @close="openCreate=false" @confirm="createEmployee">
      <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <FormInput v-model="form.dept" :label="t('employees.fields.department')" placeholder="Engineering" />
        </div>
        <div>
          <FormInput v-model="form.manager" :label="t('employees.fields.manager')" placeholder="Anh Tran" />
        </div>
      </div>
    </AppModal>

    <AppModal :open="openImport" :title="t('employees.importCsv') + ' (CSV)'" :confirmText="t('controls.upload')" @close="openImport=false" @confirm="openImport=false">
