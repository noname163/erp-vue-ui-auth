    ...
    <div class="space-y-4">
        ...
        <AppTable :columns="columns" :rows="companies" :placeholder="t('companies.searchPlaceholder')">
            ...
            <template #cell:status="{ row }">
                <span
                    class="inline-flex items-center rounded-full bg-[var(--bg-canvas)] px-2 py-1 text-xs font-medium text-[var(--text-secondary)]">
            ...
            <template #cell:actions="{ row }">
                <div class="flex gap-2">
                    <AppButton variant="ghost" size="sm" @click="openAccounts(row as Company)">{{
                        t('companies.manageAccounts') }}</AppButton>
                    <AppButton variant="ghost" size="sm" @click="startEditCompany(row as Company)">{{ t('controls.edit')
                        }}</AppButton>
                    <AppButton variant="ghost" size="sm" class="text-red-500" @click="askDeleteCompany(row as Company)">
                        {{ t('controls.remove') }}</AppButton>
                </div>
            </template>
        </AppTable>
        ...
        <AppModal :open="openDeleteModal" :title="t('companies.deleteTitle')" :confirm-text="t('controls.remove')"
            @close="closeDeleteModal" @confirm="deleteCompany">
            <p class="text-sm text-[var(--text-muted)]">
                {{ t('companies.deleteMessage', { name: companyToDelete?.name ?? '' }) }}
            </p>
        </AppModal>

        <AppModal :open="openAccountsModal"
            :title="selectedCompany ? t('companies.accounts.manageTitle', { name: selectedCompany.name }) : t('companies.accounts.title')"
            :confirm-text="t('controls.close')" @close="closeAccountsModal" @confirm="closeAccountsModal">
            <div class="space-y-4">
                <div class="space-y-2">
                    <div v-if="selectedCompany && selectedCompany.accounts.length" class="space-y-2">
                        <div v-for="account in selectedCompany.accounts" :key="account.id"
                            ...
                            <div>
                                ...
                                <div class="text-xs text-[var(--text-muted)]">{{ roleLabel(account.role) }} · {{
                                    statusLabel(account.status) }}</div>
                            </div>
                            <div class="flex gap-2">
                                <AppButton variant="ghost" size="sm" @click="startEditAccount(account)">{{
                                    t('controls.edit') }}
                                </AppButton>
                                <AppButton variant="ghost" size="sm" class="text-red-500"
                ...
                <div class="border-t border-[var(--border-subtle)] pt-3">
                    <h3 class="text-sm font-semibold mb-3">
                        {{ editingAccountId ? t('companies.accounts.editAccountTitle') :
                            t('companies.accounts.addAccountTitle')
                        }}
                    </h3>
                    <CompanyAccountForm v-model="accountForm" :role-options="accountRoleOptions"
                        :status-options="statusOptions" />
                    <div class="mt-3 flex justify-end gap-2">
                        <AppButton variant="secondary" size="md" @click="resetAccountForm()">{{
                            t('companies.accounts.resetForm') }}</AppButton>
                        <AppButton variant="primary" size="md" @click="saveAccount">
                            {{ editingAccountId ? t('companies.accounts.updateAccount') :
                                t('companies.accounts.addAccount') }}
                        </AppButton>
                    </div>
                </div>
            </div>
...
const columns = computed(() => [
    ...
    { key: 'contact', label: t('companies.columns.contact') },
